...
{   
    std::cout << "[jiaming] debug: invalid vector " << invalid_vector << std::endl;
    updateManifold();
...
void PoseConstraint::setRefPose(Eigen::Affine3d &ref_pose){
    ref_pose_=ref_pose;
}

void PoseConstraint::getRefPose(Eigen::Affine3d &ref_pose) const{
    ref_pose=ref_pose_;
}
